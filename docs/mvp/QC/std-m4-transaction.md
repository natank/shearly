...
| M4-T08 | Must | **PASS (re-run after [QCF-010](./findings.md) fix)** | `PATCH /bookings/:id/provider-cancel` added on `fix/qcf-010-provider-cancel-and-noshow-routes`. Re-run live against real Stripe: fresh `CONFIRMED` booking (`7333b1bb-adc9-4f94-9738-f113ac28b32d`) → provider cancels → `CANCELLED_BY_PROVIDER`; real Stripe PI confirmed `canceled`/`amount_received: 0` (full release, no fee); `booking.standing_events` has a `provider_cancel` row. Regression-checked the pre-existing customer `/cancel` route (T06's scenario re-run) still resolves correctly to `CustomerCancels` — no path collision. |
...
| M4-T10 | Must | **PASS (re-run after [QCF-010](./findings.md) fix)** | Steps 1-2 (customer no-show), originally run: fresh real-Stripe `CONFIRMED` booking (`7cd4b659-347b-4f27-9b04-455c39da7890`), `slot_start` pushed past; provider reports no-show → `NO_SHOW_CUSTOMER`; full capture confirmed (ledger + real Stripe `amount_received: 20000`). Steps 3-4 (provider no-show), re-run after the fix: `PATCH /bookings/:id/provider-no-show` added; fresh `CONFIRMED` booking (`332e14a8-7501-4d3f-a791-869b34e0e823`), `slot_start` pushed past → customer reports no-show → `NO_SHOW_PROVIDER`; real Stripe PI confirmed `canceled`/`amount_received: 0`; `booking.standing_events` has a `provider_no_show` row. Regression-checked: a second attempt to also mark customer no-show on the same booking correctly `409`s (terminal-state guard). |
...
**Milestone QC:** **PASS** — all 18 procedures now fully exercised. 17 PASS outright, 1 legitimately WAIVEd (T16, matching the STD's own "no manual surface, automated coverage confirmed" fallback; T15 also WAIVEd for the same reason). M4-T08 and M4-T10 (both P0/Must) were initially BLOCKED — [QCF-010](./findings.md), no HTTP route existed for `ProviderCancels`/`CustomerReportsProviderNoShow` despite both being fully implemented and tested at the state-machine layer — fixed on `fix/qcf-010-provider-cancel-and-noshow-routes` (`PATCH /bookings/:id/provider-cancel`, `PATCH /bookings/:id/provider-no-show`) and re-run live against real Stripe: both now PASS. All findings from this run are logged in [findings.md](./findings.md) as QCF-008 through QCF-013 (QCF-013: no UI exists anywhere for any provider action or no-show report — a real, deliberate M4-P4 scope cut, tracked separately, does not block this STD's PASS since the STD's procedures are API-level by design). Two defects found and fixed mid-run ([QCF-008](./findings.md), Stripe webhook route, PR #64 merged; [QCF-009](./findings.md), booking confirm's fake `paymentMethodId`, Elements now wired in), one blocking gap found and fixed ([QCF-010](./findings.md)), two Should-severity findings still open ([QCF-011](./findings.md) stale `payments.authorizations.status`; [QCF-012](./findings.md) first-run address-restoration UI gap) — neither blocks M4 exit.
