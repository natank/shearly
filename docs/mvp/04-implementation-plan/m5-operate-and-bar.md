...
|---|---|---|
| **M5-Q1** | Outbox table location — its own `events` schema, or colocated per-domain (booking's own outbox row, payments' own, etc.) per design §6.2's isolation stance? | Own `events` schema, single dispatcher — simpler operationally, and design §6.4 already frames it as one bus, not per-domain buses |
