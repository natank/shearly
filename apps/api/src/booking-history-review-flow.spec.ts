...
async function createBooking(
  ...
  const res = await app.request('/bookings', {
    ...
    body: JSON.stringify({
      providerId,
      serviceId,
      addressLine: 'qc-m4 history street',
      accessNotes: '',
      lat: 32.08,
      lng: 34.78,
...
describe('M4 booking history and review submit', () => {
  ...
  async function completeBooking(providerSession: string, bookingId: string) {
    if (!app || !services) {
    ...
    const res = await app.request(`/bookings/${bookingId}/complete`, {
      method: 'PATCH',
      headers: { cookie: providerSession },
    });
  ...
  it('history separates upcoming (soonest-first) from past (most-recent-first), with provider/service names (CUS-006)', async () => {
    if (!app || !services) {
      return;
    }
    ...
    const future1 = await createBooking(app, customer, providerId, serviceId, 9);
    await app.request(`/bookings/${future1.id}/accept`, {
      method: 'PATCH',
      headers: { cookie: providerSession },
    });
    const pastBooking = await createBooking(app, customer, providerId, serviceId, 11);
    await app.request(`/bookings/${pastBooking.id}/accept`, {
      method: 'PATCH',
      headers: { cookie: providerSession },
    });
    await completeBooking(providerSession, pastBooking.id);

    const res = await app.request('/account/me/bookings', { headers: { cookie: customer } });
    expect(res.status).toBe(200);
    const body = (await res.json()) as {
      upcoming: { id: string; providerDisplayName: string; serviceName: string }[];
      past: { id: string; state: string }[];
    };
    expect(body.upcoming.map((b) => b.id)).toContain(future1.id);
    expect(body.upcoming[0].providerDisplayName).toBe('QC Cut');
    expect(body.upcoming[0].serviceName).toBe('Cut');
    expect(body.past.map((b) => b.id)).toContain(pastBooking.id);
    expect(body.past.find((b) => b.id === pastBooking.id)?.state).toBe('COMPLETED');
  });

  it("a second customer with no bookings sees empty lists, not an error or someone else's data", async () => {
    if (!app) {
      return;
    }
    const stranger = await registerCustomer(app);
    const res = await app.request('/account/me/bookings', { headers: { cookie: stranger } });
    expect(res.status).toBe(200);
    const body = (await res.json()) as { upcoming: unknown[]; past: unknown[] };
    expect(body.upcoming).toEqual([]);
    expect(body.past).toEqual([]);
  });

  it('reviews a COMPLETED booking once; a second attempt is rejected (RAT-001)', async () => {
    if (!app || !services) {
      return;
    }
    const { providerId, serviceId, providerSession } = await seedListedProvider(app, services);
    ...
    await completeBooking(providerSession, booking.id);

    const review = await app.request(`/bookings/${booking.id}/review`, {
      method: 'POST',
      headers: { cookie: customer, 'content-type': 'application/json' },
      body: JSON.stringify({ rating: 5, body: 'lovely' }),
    });
    expect(review.status).toBe(201);

    const second = await app.request(`/bookings/${booking.id}/review`, {
      method: 'POST',
      headers: { cookie: customer, 'content-type': 'application/json' },
  ...
  it('rejects a review on a non-COMPLETED booking', async () => {
    ...
    const res = await app.request(`/bookings/${booking.id}/review`, {
      method: 'POST',
      headers: { cookie: customer, 'content-type': 'application/json' },
  ...
  it('rejects a review from a customer who does not own the booking', async () => {
    if (!app || !services) {
      return;
    }
    const { providerId, serviceId, providerSession } = await seedListedProvider(app, services);
