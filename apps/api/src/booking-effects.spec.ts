...
describe('executeEffects', () => {
  it('ReleaseAuth cancels the authorization', async () => {
    const deps = fakeDeps();
    await executeEffects(deps, 'booking-1', 'provider-1', 20000, 'ProviderDeclines', [
      { type: 'ReleaseAuth' },
    ]);
  ...
  it('Capture(100) captures the full price, then Split splits the same amount', async () => {
    const deps = fakeDeps();
    await executeEffects(deps, 'booking-1', 'provider-1', 20000, 'ProviderCompletes', [
      { type: 'Capture', pct: 100 },
      { type: 'Split' },
    ]);
    expect(deps.authorizations.capture).toHaveBeenCalledWith('booking-1', 20000);
  ...
  it('Capture(50) then Split splits only the 50% actually captured, not the full price', async () => {
    const deps = fakeDeps();
    await executeEffects(deps, 'booking-1', 'provider-1', 20000, 'CustomerCancels', [
      { type: 'Capture', pct: 50 },
      { type: 'Split' },
    ]);
    expect(deps.authorizations.capture).toHaveBeenCalledWith('booking-1', 10000);
  ...
  it('Refund(50) refunds half the price with the given reason', async () => {
    ...
    expect(deps.authorizations.refund).toHaveBeenCalledWith('booking-1', 10000, 'CustomerCancels');
  });

  it('Refund(100) then Split splits the full refunded amount', async () => {
    const deps = fakeDeps();
    await executeEffects(deps, 'booking-1', 'provider-1', 20000, 'ProviderCancels', [
      { type: 'Refund', pct: 100 },
    ]);
    expect(deps.authorizations.refund).toHaveBeenCalledWith('booking-1', 20000, 'ProviderCancels');
  });
  ...
  it('ReleaseHold and Notify are no-ops that do not throw', async () => {
    const deps = fakeDeps();
    await expect(
      executeEffects(deps, 'booking-1', 'provider-1', 20000, 'ProviderAccepts', [
        { type: 'ReleaseHold' },
  ...
  it("processes effects in order, so Split after Capture uses that Capture's amount", async () => {
    ...
    await executeEffects(deps, 'booking-1', 'provider-1', 20000, 'AutoCompleteElapsed', [
      { type: 'Split' },
    ]);
    expect(deps.ledger.split).toHaveBeenCalledWith('booking-1', 20000);
  });
